...
If ANSI colors are not support, but escape codes are, then it will resort to xterm's base 16 colors with bright and dark variants. 

If escape codes are not support either, then it will resort to .NET's basic system colors by utilization `ConsoleColor`.
